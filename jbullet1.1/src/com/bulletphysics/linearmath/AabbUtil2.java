...
public class AabbUtil2
{
	...
	//deburners
	private static Vector3f aabbHalfExtent = new Vector3f();
	private static Vector3f aabbCenter = new Vector3f();
	private static Vector3f source = new Vector3f();
	private static Vector3f target = new Vector3f();
	private static Vector3f r = new Vector3f();
	private static Vector3f hitNormal = new Vector3f();
	...
	public static boolean testTriangleAgainstAabb2(Vector3f[] vertices, Vector3f aabbMin, Vector3f aabbMax)
	{
		...
		if (Math.min(Math.min(p1.y, p2.y), p3.y) > aabbMax.y)
			return false;
		if (Math.max(Math.max(p1.y, p2.y), p3.y) < aabbMin.y)
			return false;
	...
	public static void transformAabb(Vector3f localAabbMin, Vector3f localAabbMax, float margin, Transform trans, Vector3f aabbMinOut,
			Vector3f aabbMaxOut)
	{
